...
RoutingProtocol::RoutingProtocol ()
  : m_rreqRetries (2),
    ...
    m_rreqCount (0),
    m_rerrCount (0),
    m_flag(1),   //  flag(1/0) for  packet drop/pass  behavior  :by_GopalRawat
...
/*
void
RoutingProtocol::PrintRoutingNeigh (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const  //GSR
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        << "; Time: " << Now ().As (unit)
                        << ", Local time: " << GetObject<Node> ()->GetLocalTime ().As (unit)
                        << ", AODV Neigh table" << std::endl;

  m_routingTable.PrintNeigh (stream);
  *stream->GetStream () << std::endl;
}
*/
int64_t
...
/* Code added by Shalini Satre, Wireless Information Networking Group (WiNG), NITK Surathkal for simulating Blackhole Attack */
  /* Check if the node is suppose to behave maliciously */
        if(IsMalicious )
          {
              if (m_flag == 1) // flag =1 it acts as malicous node ; flag =0 acts as normal node  : by_GopalRawat
              {  // When malicious node receives packet it drops the packet.
                std :: cout << "[node " << m_ipv4->GetObject<Node> ()->GetId () << "] "<<"Launching Packet Drop Attack! Packet dropped . . . \n";
                NS_LOG_DEBUG("Launching Blackhole Attack! Packet dropped");
                m_flag = 0;
                return false; 
              }
              // malicious node behaves normal alternatively  : by_GopalRawat
              std :: cout<<"[node " << m_ipv4->GetObject<Node> ()->GetId () << "] "<<" Acts normal . . . \n";   // Gopal Rawat
              m_flag =1; 
              NS_LOG_DEBUG("Intelligent Malicious Behavior!");         
          }
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if ((IsMalicious && m_flag==1) || m_routingTable.LookupRoute (dst, toDst))
    {
      /*
       ...
       */
      if ((IsMalicious && m_flag==1) || ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo () ))
        {
          if ((IsMalicious && m_flag==1) || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              m_routingTable.LookupRoute (origin, toOrigin); 
              /* Code added by Shalini Satre, Wireless Information Networking Group (WiNG), NITK Surathkal for simulating Blackhole Attack
              ...
              */
              ...
              if((IsMalicious && m_flag==1))
              {  // flag =1 it acts as malicous node ; flag =0 acts as normal node
                 {
                   Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
...
RoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep)
{
  ...
  if((IsMalicious && m_flag==1))                       //Shalini Satre ( Edit Gopal Rawat)
  {  
     {
